...
//  TODO: maybe put missing values here so that indices correspond to those from
//  MU[]?
const double RPL[6] = {
    2440,  // Mercury
    6052,  // Venus
...
/**
 * Compute velocity and position of an celestial object of interest at specified
 * time.
 *
...
void get_celobj_r_and_v(const mgadsmproblem &problem, const double T,
  ...
  if (problem.sequence[i_count] < 10) { // normal planet
    Planet_Ephemerides_Analytical(
        T, problem.sequence[i_count], r,
        v); // r and  v in heliocentric coordinate system
  } else {  // asteroid
...
/**
 * Precomputes all velocities and positions of celestial objects of interest for
 * the problem.
 * Before calling this function, r and v verctors must be pre-allocated with
 * sufficient amount of entries.
 *
 * problem - concerned problem
...
void first_block(const vector<double> &t, const mgadsmproblem &problem,
  ...
  // [MR] {LITTLE HACKER TRICK} Instead of copying (!) arrays let's just
  // introduce pointers to appropriate positions in the decision vector.
  ...
  phi = acos(2 * vdir - 1) - M_PI / 2; // In this way: -pi/2<phi<pi/2 so phi
                                       // can be used as out-of-plane rotation

  double vinf[3];
  for (i = 0; i < 3; i++)
    vinf[i] = VINF * (cos(theta) * cos(phi) * iP1[i] +
                      sin(theta) * cos(phi) * jP1[i] + sin(phi) * zP1[i]);

  // double v_sc_pl_in[3];  // Spacecraft absolute incoming velocity at P1 [MR]
  // not needed?
  ...
  LambertI(rd, r[1], tof[0] * (1 - alpha[0]) * 86400, MU[0], lw, v_sc_dsm_out,
           v_sc_nextpl_in, a, p, theta2,
           iter_unused); // [MR] last 6 are output

  // First Contribution to DV (the 1st deep space maneuver)
  for (i = 0; i < 3; i++) {
    Dum_Vec[i] = v_sc_dsm_out[i] -
                 v_sc_dsm_in[i]; // [MR] Temporary variable reused. Dirty.
  }
...
void intermediate_block(const vector<double> &t, const mgadsmproblem &problem,
                        ...
                        const std::vector<double *> &v, int i_count,
  ...
  const double *tof = &t[4];
  const double *alpha = &t[n + 3];
  const double *rp_non_dim = &t[2 * n + 2]; // non-dim perigee fly-by radius of
                                            // planets P2..Pn(-1) (i=1 refers
                                            // to the second planet)
  const double *gamma =
      &t[3 * n]; // rotation of the bplane-component of the swingby outgoing
  ...
  double e =
      1.0 +
      rp_non_dim[i_count] * RPL[sequence[i_count + 1] - 1] * vrelin / hopobj_mu;
  ...
  LambertI(rd, r[i_count + 2],
           ...
           v_sc_dsm_out, v_sc_nextpl_in, a, p, theta,
           iter_unused); // [MR] last 6 are output.
...
// FINAL BLOCK
//
void final_block(const mgadsmproblem &problem, const std::vector<double *> &r,
                 const std::vector<double *> &v, const double v_sc_pl_in[],
  ...
  if ((problem.type == orbit_insertion) ||
    ...
    double DVper = sqrt(DVrel * DVrel +
                        2 * MU[sequence[n - 1]] /
                            rp_target); //[MR] should MU be changed to get_... ?
    double DVper2 = sqrt(2 * MU[sequence[n - 1]] / rp_target -
...
int MGA_DSM(
    /* INPUT values: */ //[MR] make this parameters const, if they are not
                        // modified and possibly references (especially
    //'problem').
    vector<double> t, // it is the vector which provides time in modified
                      // julian date 2000. [MR] ??? Isn't it the decision vetor
                      // ???
  ...
  inter_pl_in_v[2] = inter_pl_out_v[2];
  // FINAL BLOCK
  final_block(problem, r, v, inter_pl_in_v, DV);
  ...
  const double &VINF =
      t[1]; // Variable renaming: Hyperbolic escape velocity (km/sec)
  ...
  if (problem.type == orbit_insertion) {
    ...
    // [MR] helper constants
    const vector<int> &sequence = problem.sequence;
    const double *rp_non_dim = &t[2 * n + 2]; // non-dim perigee fly-by radius
                                              // of planets P2..Pn(-1) (i=1
                                              // refers to the second planet)
    const double *gamma =
        &t[3 * n]; // rotation of the bplane-component of the swingby outgoing
    ...
    double e =
        ...
        rp_non_dim[n - 2] * RPL[sequence[n - 2 + 1] - 1] * vrelin /
            get_celobj_mu(problem,
                          n - 1); // I hope the planet index (n - 1) is OK
    ...
    // ix=r_rel_in/norm(v_rel_in);  // activating this line and disactivating
    // the one above
