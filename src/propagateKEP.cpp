...
void propagateKEP(const double *r0_in, const double *v0_in, double t, double mu,
  ...
  */
  ...
  if (fabs(fabs(ih[2]) - 1.0) <
      1e-3) // the abs is needed in cases in which the orbit is retrograde,
  {         // that would held ih=[0,0,-1]!!
    ...
    // Random rotation matrix that make the Euler angles well defined for the
    // case
    ...
    // for practical reason we take the transpose of the matrix DD here (will be
...
/*
        ...
        The algorithm used is quite common and can be found as an example in
   Bate,
...
void IC2par(const double *r0, const double *v0, double mu, double *E) {
  ...
  if (e < 1.0)
    E[5] = 2.0 * atan(sqrt((1 - e) / (1 + e)) *
                      tan(ni / 2.0)); // algebraic kepler's equation
  else
    E[5] = 2.0 * atan(sqrt((e - 1) / (e + 1)) *
...
/*
        ...
        Comments:  The parameters returned are, of course, referred to the same
        ref. frame in which r0,v0 are given. a can be given either in kms or
   AUs,
