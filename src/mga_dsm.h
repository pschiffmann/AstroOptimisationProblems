...
struct mgadsmproblem {
  ...
  vector<int> sequence; // fly-by sequence (ex: 3,2,3,3,5,is
  ...
  customobject asteroid; // asteroid data (in case fly-by sequence has a final
                         // number = 10)
  ...
  double DVonboard; // Total DV on the spacecraft in km/s (only in case of
                    // time2AUs)

  // Pre-allocated memory, in order to remove allocation of heap space in
  // MGA_DSM calls
  // The DV vector serves also as output containing all the values for the
  // impulsive DV
...
int MGA_DSM(
    ...
    mgadsmproblem &mgadsm, // contains the problem specific data, passed as
                           // reference as mgadsm.DV is an output
