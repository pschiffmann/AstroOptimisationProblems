 ...
 *  Copyright 2008 ¿dvanced Concepts Team (European Space Agency). All rights
 * reserved.
 *
 */
...
double gtoc1(const vector<double> &x, std::vector<double> &rp) {
  ...
  const int rev_[GTOC1_DIM] = {0, 0, 0, 0,
                               0, 0, 1, 0}; // sequence of clockwise legs
...
double cassini1(const vector<double> &x, std::vector<double> &rp) {
  ...
  const int rev_[CASSINI_DIM] = {0, 0, 0,
                                 0, 0, 0}; // sequence of clockwise legs
...
double sagas(const vector<double> &x, double &DVtot, double &DVonboard) {
  ...
  DVtot = problem.DV[0] + problem.DV[1] + problem.DV[2] + problem.DV[3] +
          problem.DV[4];
...
double tandem(const vector<double> &x, double &tof, const int sequence_[]) {
  ...
  double zP1[3];
  double normvtemp =
      sqrt(vtemp[0] * vtemp[0] + vtemp[1] * vtemp[1] + vtemp[2] * vtemp[2]);
  ...
  double phi = acos(2 * vdir - 1) - M_PI / 2; // In this way: -pi/2<phi<pi/2 so
                                              // phi can be used as
                                              // out-of-plane rotation
  double vinf[3];
  vinf[0] = VINFE * (cos(theta) * cos(phi) * iP1[0] +
                     sin(theta) * cos(phi) * jP1[0] + sin(phi) * zP1[0]);
  vinf[1] = VINFE * (cos(theta) * cos(phi) * iP1[1] +
                     sin(theta) * cos(phi) * jP1[1] + sin(phi) * zP1[1]);
  vinf[2] = VINFE * (cos(theta) * cos(phi) * iP1[2] +
                     sin(theta) * cos(phi) * jP1[2] + sin(phi) * zP1[2]);
  // We rotate it to the equatorial plane
  ecl2equ(vinf, vinf);
  // And we find the declination in degrees
  double normvinf =
      sqrt(vinf[0] * vinf[0] + vinf[1] * vinf[1] + vinf[2] * vinf[2]);
