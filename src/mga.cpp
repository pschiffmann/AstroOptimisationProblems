...
int MGA(vector<double> t, // it is the vector which provides time in modified
                          // julian date 2000.
        // The first entry is launch date, the next entries represent the time
        // needed to
...

{
  ...
  const vector<int> rev_flag =
      problem.rev_flag; // array containing 0 clockwise, 1 un-clockwise
  ...
  if (n >= 2) {
    ...
    for (i_count = 1; i_count <= n - 2; i_count++) {
      ...
      /*if (i_count%2 != 0)	{*/
      LambertI(r[i_count], r[i_count + 1], t[i_count + 1] * 24 * 60 * 60, MU[0],
               lw,                                             // INPUT
  ...
  if (problem.type == total_DV_orbit_insertion) {
    ...
    DVper2 = sqrt(2 * MU[sequence[n - 1]] / rp_target -
