  ...
  if (r0norm < rtarget) {
    ...
    if (e < 1) {
      ...
      else         // we find the anomaly where the target distance is reached
      {
        ...
        Et = 2 * atan(sqrt((1 - e) / (1 + e)) *
                      tan(ni / 2)); // algebraic kepler's equation
    ...
    {
      ni = acos((p / rtarget - 1) / e); // in 0-pi
      Et = 2 * atan(sqrt((e - 1) / (e + 1)) *
                    tan(ni / 2)); // algebraic equivalent of kepler's equation
                                  // in terms of the Gudermannian

      if (temp > 0) // out==1
        return sqrt(pow((-a), 3)) * (e * tan(Et) - log(tan(Et / 2 + M_PI / 4)) -
